...
/// padding.
VORTEX_PREFIX Slice mem_alloc_slice(Allocator self, usize len) {
    return (Slice){.ptr = mem_alloc(self, len), .len = len};
}

/// Resizes in place.
VORTEX_PREFIX void *mem_resize(Allocator self, void *buf, usize len, usize new_len) {
    return self.realloc(self.ptr, buf, mem_align_len(len), mem_align_len(new_len));
}
/// Resizes the slice in place.
VORTEX_PREFIX Slice mem_resize_slice(Allocator self, Slice slice, usize new_len) {
    return (Slice){.ptr = mem_resize(self, slice.ptr, slice.len, new_len), .len = new_len};
}

/// Frees the memory.
VORTEX_PREFIX void mem_free(Allocator self, void *buf, usize len) {
    self.realloc(self.ptr, buf, mem_align_len(len), 0);
}
/// Frees the slice.
VORTEX_PREFIX void mem_free_slice(Allocator self, Slice slice) {
    mem_free(self, slice.ptr, slice.len);
}

/// Resizes in place or allocates a new one if failed.
VORTEX_PREFIX void *mem_resize_or_alloc(Allocator self, void *buf, usize len, usize new_len) {
    void *allocated;
    void *resized = mem_resize(self, buf, len, new_len);
    if (resized != nullptr) return resized;

    allocated = mem_alloc(self, new_len);
...
VORTEX_PREFIX Slice mem_resize_or_alloc_slice(Allocator self, Slice slice, usize new_len) {
    return (Slice){.ptr = mem_resize_or_alloc(self, slice.ptr, slice.len, new_len), .len = new_len};
}

/// Duplicates the memory.
VORTEX_PREFIX void *mem_dupe(Allocator self, void *buf, usize len) {
    void *allocated = mem_alloc(self, mem_align_len(len));
    mem_copy(allocated, buf, len);
    return allocated;
}
/// Duplicates the slcie.
VORTEX_PREFIX Slice mem_dupe_slice(Allocator self, Slice slice) {
